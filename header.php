<?php
 
 
?>


<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>header</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">
    <link rel="stylesheet" href="main1.css">

    <link rel="icon" type="image/x-icon" href="data:image/jpeg;base64,">

  <style>
    .navbar{
      color:white !important;
    }
  </style>
  </head>

<body>
  
<div class="mynav" style="color: white;"><nav class="navbar navbar-expand-lg ">
        <div class="container-fluid">
          <a class="navbar-brand"  href="#">
            <div clas="main-heading" id="main-heading">ALL-IN-<span class="one">1 </span>Tutorial &nbsp;&nbsp;&nbsp;</div>
          </a>
          <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavDropdown" aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
          </button>
          <div class="collapse navbar-collapse" id="navbarNavDropdown">
            <ul class="navbar-nav">
              <li class="nav-item">
                <a class="nav-link " id="nav1" aria-current="page" onclick="selectNavItem(this)" href="main1.php">Home</a>
              </li>
             
         <?php   
         
if(!isset($_SESSION['loggedin']) || $_SESSION['loggedin']!=true)  {
         echo  '<li id="nav2" class="nav-item">
                <a class="nav-link" href="login.php?discuss=Group Discussion"> Discuss</a><p class="dropdown-item-hover">please login to access</p>
              </li>';}

if (isset($_SESSION['loggedin']) && $_SESSION['loggedin']) {
  echo  '<li id="nav2" class="nav-item">
  <a class="nav-link" onclick="selectNavItem(this)" href="comment.php"> Discuss</a>
</li>';}
              ?>

<?php   
         
         if(!isset($_SESSION['loggedin']) || $_SESSION['loggedin']!=true)  {
              echo '<li class="nav-item" 
              // style="color:white;"
              >
                <a id="nav3" class="nav-link" onclick="selectNavItem(this)" href="login.php">Log in</a>
              </li>';
         }  ?>
<?php
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin']) {
              echo'<li class="nav-item">
                <a id="nav3" onclick="selectNavItem(this)" class="nav-link" href="logout.php">Log out</a>
              </li>';}?>
              <li id="nav4"class="nav-item dropdown">
                <a  class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                  Available
                </a>
                <ul class="dropdown-menu">
          
       
<?php 
// Check if the user is logged in
if(!isset($_SESSION['loggedin']) || $_SESSION['loggedin']!=true)  {
        echo'<li><a class="dropdown-item" href="login.php?available=Project">Project</a> <p class="dropdown-item-hover">please login to access</p><li>';
        echo '<li><a class="dropdown-item" href="login.php?available=Notes">Notes</a> <p class="dropdown-item-hover">please login to access</p></li>';
        echo ' <li><a class="dropdown-item" href="login.php?available=Important Questions">Important Questions</a> <p class="dropdown-item-hover">please login to access</p></li>';
}      
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin']) {
  echo'<li><a class="dropdown-item" onclick="selectNavItem(this)" href="welcome.php">Project</a> <li>';
        echo '<li><a class="dropdown-item" onclick="selectNavItem(this)" href="welcome.php">Notes</a> </li>';
        echo ' <li><a class="dropdown-item" onclick="selectNavItem(this)" href="welcome.php">Important Questions</a> </li>';

}            
             ?>
                </ul>
              </li>
            </ul>
          </div>
        </div>
      </nav>
    </div>



    </body>
    <script>
      function selectNavItem(element) {
  // Remove 'active' class from all nav items
  const navItems = document.querySelectorAll('.nav-item');
  navItems.forEach(item => item.classList.remove('active'));

  // Add 'active' class to the clicked nav item
  element.classList.add('active');
}

    </script>
    </html>